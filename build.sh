...
SRC_SUB_DIRS=". gui gui/compound gui/objects gui/style particle rendering rendering/renderer rendering/renderer/gl3 rendering/renderer/gles2 rendering/renderer/gles3 scene scene/model"
...
if [ $BUILD_OS != "osx" -a $BUILD_OS != "ios" ]; then
	...
	PACKAGES="sdl2 SDL2_image freetype2"
	PACKAGES_OPT=""
	...
	INCLUDES="${INCLUDES} -isystem /opt/X11/include -isystem /opt/X11/include/freetype2"
	if [ ${BUILD_CONF_XML} -gt 0 ]; then
	...
	# frameworks and libs
	LDFLAGS="${LDFLAGS} -framework SDL2 -framework SDL2_image -lfreetype"
	if [ ${BUILD_CONF_XML} -gt 0 ]; then
...
if [ $BUILD_OS == "osx" -o $BUILD_OS == "ios" ]; then
	...
	# set lib version
	# TODO: get this from the version header file
