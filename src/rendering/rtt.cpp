		...
		for(unsigned int i = 0; i < buffers.back()->attachment_count; i++) {
			...
			switch(buffers.back()->target[i]) {
...
#endif
				case GL_TEXTURE_2D:
					glTexImage2D(buffers.back()->target[i], 0, texman::convert_internal_format(internal_format[i]), width, height, 0, format[i], type[i], NULL);
					break;
				/*case GL_TEXTURE_3D:
					// TODO: tex3d
				...
				case GL_TEXTURE_2D_MULTISAMPLE:
					glTexImage2DMultisample(buffers.back()->target[i], (GLsizei)get_sample_count(buffers.back()->anti_aliasing[0]), texman::convert_internal_format(internal_format[i]), width, height, false);
					break;
#endif
